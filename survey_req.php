...
<form name="survey" enctype="multipart/form-data" action="rec_survey_parts.php?sid=<?php echo $survey_id;?>" method="post">
	...
	<table width="1200" cellspacing="0" >
    	<tr>
        	<td colspan="10" align="center"><h8 style="color:#FFF">Requisition List # <?php  if(trim($survey_id)!==''){	echo str_pad($survey_id,5,'0',STR_PAD_LEFT);} else echo 'New';?></h8></td>
      </tr>
        ...
        <tr>
        	<td class="middle-left-child" width="20%">Name</td>
            <td class="middle-none-child" width="20%">Address</td>
            <td class="middle-none-child" width="15%">Particlular</td>
            <td class="middle-none-child" width="10%">Amount</td> 
            <td class="middle-none-child" width="15%">Reference</td> 
            <td class="middle-none-child" width="15%">Remarks</td> 
            <td class="middle-right-child" colspan="4"></td>
        </tr>
        <tr>
        	<td class="middle-left-child"><input type="text" name="name" style="background-color:#FAD090;" maxlength="100" size="30"/></td>
            <td class="middle-none-child"><input type="text" name="address1" style="background-color:#FAD090;" maxlength="100" size="30"/></td>
            <td class="middle-none-child">
            <select name="particular" style="background-color:#FAD090">
            <?php
				$sql10 = "SELECT * FROM [dbo].[ClaimCodePart]";
				$rs10= sqlsrv_query($conn,$sql10);
				while($row10 = sqlsrv_fetch_array($rs10)){
					echo '<option value="'.$row10['Particular'].'">'.$row10['Particular'].'</option>';	
				}
            ...
            <td class="middle-none-child"><input type="text" name="amount" style="background-color:#FAD090;" maxlength="10" size="10"/></td> 
        ...
        <?php
			...
			while($row5=$rs5->fetch_assoc()){
				if(trim($row5['transportation'])!== ''){
					...
					$pprice=$currency.number_format($row5['price'],2);
